...
| Product-level sold quantity | `sold_quantity` in `GET /products/{product_id}` | Under live revalidation | Yes | Accumulated quantity when present | Must not be interpreted as the number of sales used to calculate the bestseller position. Availability can vary by product/site. |
